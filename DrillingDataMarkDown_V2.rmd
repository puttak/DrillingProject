...
```{r}
load("TransformationSummaryFiles_V2.RData") 
dfImp = df_Importance
...
List of Variables from Kurt
```{r}
## All removed variables with reason
df_Removed = rbind(dfRemAccumtCols, dfRemConstCols, dfRemExploitCols, dfRemMissCols)
df_Removed

KurtVariables = c("HoleDepth","ADROP","BitHours","BitPosition","BitTVD","GammaRay","HookLoad","PumpOutputTTL","ROPAverage","ROPFast","SoftSpeedEstimatedBHASpeed","SoftSpeedStickSlipSeverity","SvyInclination","TopDriveRPM",
                  "TopDriveTorque","WeightOnBit","SoftSpeedActive","DiffPressure","PumpPressure","BitSize",
                  "TVD")
KurtVariables
myVariables = names(df_1_31862)
myVariables
lostVars = KurtVariables[!(KurtVariables %in% myVariables)]
lostVars

# where are the lost variables 
df_Removed[df_Removed$FeatureName %in% lostVars, ]

## extra variables
extraVars = myVariables[!(myVariables %in% KurtVariables)]
extraVars
```
